...
App.ui.Arena = (() => {
  ...
  function pickMatch() {
    ...
    let songA =
      newbies.length > 0 && Math.random() < 0.6
    ...
    if (Math.random() < 0.5) {
      others.sort(
        (a, b) =>
          Math.abs(a.points - songA.points) - Math.abs(b.points - songA.points),
      );
      songB = others[0];
    } else {
  ...
  function startNewMatch(withAnimation = false) {
    ...
    hydratePreviews(match).then(() => {
      if (App.state.currentMatch === match) {
        render(false);
      }
    });

    if (localStorage.getItem("peakr_fastmode") === "true") {
  ...
  async function hydratePreviews(match) {
    const promises = match.map(async (song) => {
      if (song.useDeezer && !song.previewUrl) {
        const url = await App.deezer.fetchPreview(
          song.searchArtist || song.artist,
          song.searchTitle || song.display,
        );
        if (url) {
  ...
  async function processVote(winnerIdx) {
    ...
    if (currentCardElements) {
      const cardW =
        winnerIdx === 0 ? currentCardElements.el1 : currentCardElements.el2;
      const cardL =
        winnerIdx === 0 ? currentCardElements.el2 : currentCardElements.el1;
    ...
    if (currentCardElements) {
      ...
      showChangeOverlay(
        cardW,
        changeW > 0 ? `+${changeW}` : `${changeW}`,
        changeW > 0,
      );
      showChangeOverlay(
        cardL,
        changeL > 0 ? `+${changeL}` : `${changeL}`,
        changeL > 0,
      );

      const scoreW =
        winnerIdx === 0
          ? currentCardElements.score1
          : currentCardElements.score2;
      const scoreL =
        winnerIdx === 0
          ? currentCardElements.score2
          : currentCardElements.score1;
    ...
    if (newWTier.id !== oldWTier)
      App.ui.Toast.show(`🚀 ${dbWinner.display} ascendió a ${newWTier.name}!`);
    if (newLTier.id !== oldLTier)
      App.ui.Toast.show(`📉 ${dbLoser.display} bajó a ${newLTier.name}`);

    startNewMatch(true);
  }

  return { refresh, render, skipMatch };
})();
