...
App.ui.Ranking = (() => {
  function render() {
    ...
    if (App.state.db.songs.length === 0) {
      podiumEl.innerHTML = '';
      listEl.innerHTML   = '<p style="text-align:center;color:var(--text-sec);padding:48px 0">No hay canciones aún</p>';
      return;
    }
    ...
    sorted.slice(3).forEach((song, i) => {
      ...
      item.innerHTML =
        `<div class="ranking-left-card" style="background-image: url('${App.ui.escHtml(song.imgUrl || App.state.DEFAULT_IMG)}')">
      ...
      leftCard.addEventListener('click', () => {
        if (song.link) chrome.tabs.create({ url: song.link });
      });
  ...
  function renderPodium(container, top) {
    ...
    slots.forEach(slot => {
      ...
      el.addEventListener('click', () => {
        if (slot.song.link) chrome.tabs.create({ url: slot.song.link });
      });
